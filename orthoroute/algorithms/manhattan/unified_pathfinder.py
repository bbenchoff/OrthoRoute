...
═══════════════════════════════════════════════════════════════════════════════
(moved to manhattan/hotset_policy.py, next to the code it describes)
...
═══════════════════════════════════════════════════════════════════════════════
(moved to manhattan/via_accounting.py, next to the code it describes)
...
from .geometry_emitter import GeometryEmitter, GeometryPayload
from .via_accounting import ViaAccounting
from .hotset_policy import HotsetPolicy
...
class PathFinderRouter:
    ...
    # Collaborators (Phase 8 decomposition). cached_property rather than
    # __init__ assignment so routers constructed via object.__new__ in tests
    # still get working delegation.
    @functools.cached_property
    def _geometry_emitter(self):
        return GeometryEmitter(self)

    @functools.cached_property
    def _via_accounting(self):
        return ViaAccounting(self)
    ...
    def _accumulate_via_usage_for_path(
        ...
        *,
        ...
        return self._via_accounting._accumulate_via_usage_for_path(node_path, net_id, col_use=col_use, seg_use=seg_use)

    def _rebuild_via_usage_from_committed(self):
        return self._via_accounting._rebuild_via_usage_from_committed()
    ...
    def _add_via_contribution(self, net_id: str, path: list):
        return self._via_accounting._add_via_contribution(net_id, path)

    def _remove_via_contribution(self, net_id: str):
        return self._via_accounting._remove_via_contribution(net_id)
    ...
    def _mark_via_barrel_ownership_for_path(self, net_name: str, path: List[int]) -> None:
        return self._via_accounting._mark_via_barrel_ownership_for_path(net_name, path)

    def _clear_via_barrel_ownership_for_path(
        self, net_name: str, path: List[int]
    ) -> None:
        return self._via_accounting._clear_via_barrel_ownership_for_path(net_name, path)
    ...
    def _via_nodes_for_hop(self, u: int, v: int) -> List[int]:
        return self._via_accounting._via_nodes_for_hop(u, v)

    def _via_nodes_for_path(self, path: List[int]) -> List[int]:
        return self._via_accounting._via_nodes_for_path(path)
    ...
    def _apply_via_pooling_penalties(self, pres_fac: float):
        return self._via_accounting._apply_via_pooling_penalties(pres_fac)

    def _apply_via_pooling_penalties_sequential(self, pres_fac: float):
        return self._via_accounting._apply_via_pooling_penalties_sequential(pres_fac)

    def _spatial_via_overuse_total(self) -> int:
        return self._via_accounting._spatial_via_overuse_total()

    def _block_via_edges_with_collisions(self):
        return self._via_accounting._block_via_edges_with_collisions()
    ...
    def _history_hotset_cap(total_overuse: float) -> int:
        return HotsetPolicy._history_hotset_cap(total_overuse)
    ...
    def _rolling_progress_insufficient(
        ...
        return HotsetPolicy._rolling_progress_insufficient(values, window, minimum_fraction, minimum_overuse)
    ...
    def _pressure_work_scale(
        ...
        return HotsetPolicy._pressure_work_scale(routed_task_count, reference_hotset, maximum_scale)
    ...
    def _effective_history_hotset_cap(
        ...
        return HotsetPolicy(self)._effective_history_hotset_cap(total_overuse)

    def _bounded_history_hotset_cap(
        self,
        total_overuse: float,
    ) -> int:
        return HotsetPolicy(self)._bounded_history_hotset_cap(total_overuse)

    @staticmethod
    def _hotset_exploration_fraction(total_overuse: float) -> float:
        return HotsetPolicy._hotset_exploration_fraction(total_overuse)
    ...
    def _select_conflict_aware_hotset(
        ...
        return HotsetPolicy._select_conflict_aware_hotset(ranked_candidates, conflict_pairs, cap, exploration_fraction, rng)
    ...
    def _should_rip_for_stagnation(
        ...
        return HotsetPolicy._should_rip_for_stagnation(spatial_via_overuse, tail_threshold, physical_cleanup_started)
    ...
    def _select_physical_hotset(self) -> Set[str]:
        return HotsetPolicy(self)._select_physical_hotset()
    ...
    def _build_hotset(self, tasks: Dict[str, Tuple[int, int]], ripped: Optional[Set[str]] = None) -> Set[str]:
        return HotsetPolicy(self)._build_hotset(tasks, ripped)
    ...
    def _rank_stagnation_offenders(
        ...
        return HotsetPolicy(self)._rank_stagnation_offenders(over)

    def _rip_top_k_offenders(self, k=20) -> Set[str]:
        return HotsetPolicy(self)._rip_top_k_offenders(k)
    ...
    def _select_stagnation_victims(
        ...
        return HotsetPolicy(self)._select_stagnation_victims(scores, k)
    ...
    def _identify_via_edges(self):
        return self._via_accounting._identify_via_edges()

    def _build_via_edge_metadata(self):
        return self._via_accounting._build_via_edge_metadata()
    ...
    def _detect_barrel_conflicts(self) -> Tuple[np.ndarray, int]:
        return self._via_accounting._detect_barrel_conflicts()
    ...
    def _segment_world(self, a_idx: int, b_idx: int, layer: int, net: str):
        return self._geometry_emitter._segment_world(a_idx, b_idx, layer, net)
    ...
    def _via_world(self, at_idx: int, net: str, from_layer: int, to_layer: int):
        return self._geometry_emitter._via_world(at_idx, net, from_layer, to_layer)

    def _expand_hdi_vias(self, vias: List[dict]) -> List[dict]:
        return self._geometry_emitter._expand_hdi_vias(vias)

    def _refresh_selected_escape_geometry(self) -> None:
        return self._geometry_emitter._refresh_selected_escape_geometry()

    def emit_geometry(self, board: Board) -> Tuple[int, int]:
        return self._geometry_emitter.emit_geometry(board)

    def _path_without_dynamic_escape_chains(self, net_id: str, path: List[int]) -> List[int]:
        return self._geometry_emitter._path_without_dynamic_escape_chains(net_id, path)

    def _generate_geometry_from_paths(self) -> Tuple[List, List]:
        return self._geometry_emitter._generate_geometry_from_paths()

    def _coalesce_vertical_runs(self, path: List[int]) -> List[int]:
        return self._geometry_emitter._coalesce_vertical_runs(path)

    def get_geometry_payload(self):
        return self._geometry_emitter.get_geometry_payload()

    def get_provisional_geometry(self):
        return self._geometry_emitter.get_provisional_geometry()
